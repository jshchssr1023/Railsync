...
for (let i = 1; i < lines.length; i++) {
  ...
  const adjStatus = esc(v[idx['Adjusted Status']], 30);
  const planStatus = esc(v[idx['Plan Status']], 30);

  sql += `INSERT INTO cars (car_number, lessee_name, car_mark, lessee_code, contract_number, contract_expiration, commodity, car_age, car_type, is_jacketed, is_lined, lining_type, interior_lining_year, rule_88b_year, safety_relief_year, service_equipment_year, stub_sill_year, tank_thickness_year, tank_qual_year, portfolio_status, current_status, adjusted_status, plan_status) VALUES ('${carNumber}', '${lesseeName}', '${carMark}', '${lesseeCode}', '${contract}', ${contractExp}, '${commodity}', ${carAge}, '${carType}', ${jacketed}, ${lined}, '${liningType}', ${interior}, ${rule88b}, ${safety}, ${service}, ${stub}, ${tankThick}, ${tankQual}, '${portfolio}', '${curStatus}', '${adjStatus}', '${planStatus}') ON CONFLICT (car_number) DO UPDATE SET lessee_name=EXCLUDED.lessee_name, car_mark=EXCLUDED.car_mark, lessee_code=EXCLUDED.lessee_code, contract_number=EXCLUDED.contract_number, contract_expiration=EXCLUDED.contract_expiration, commodity=EXCLUDED.commodity, car_age=EXCLUDED.car_age, car_type=EXCLUDED.car_type, is_jacketed=EXCLUDED.is_jacketed, is_lined=EXCLUDED.is_lined, lining_type=EXCLUDED.lining_type, interior_lining_year=EXCLUDED.interior_lining_year, rule_88b_year=EXCLUDED.rule_88b_year, safety_relief_year=EXCLUDED.safety_relief_year, service_equipment_year=EXCLUDED.service_equipment_year, stub_sill_year=EXCLUDED.stub_sill_year, tank_thickness_year=EXCLUDED.tank_thickness_year, tank_qual_year=EXCLUDED.tank_qual_year, portfolio_status=EXCLUDED.portfolio_status, current_status=EXCLUDED.current_status, adjusted_status=EXCLUDED.adjusted_status, plan_status=EXCLUDED.plan_status;\n`;
  count++;
}
