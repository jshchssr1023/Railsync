  ...
  return (
    <>
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={shop.shop.shop_name}
      >
        {/* Header */}
        <div className="sticky top-0 bg-white dark:bg-gray-900 border-b dark:border-gray-700 px-6 py-4 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">{shop.shop.shop_name}</h2>
            <div className="flex items-center gap-2 mt-1">
              <span className="text-sm text-gray-500 dark:text-gray-400">{shop.shop.shop_code}</span>
              <span className="text-gray-300 dark:text-gray-600 dark:text-gray-400">|</span>
              <span className="text-sm text-gray-500 dark:text-gray-400">{shop.shop.primary_railroad}</span>
        ...
        <div className="overflow-y-auto h-[calc(100%-140px)] px-6 py-4 space-y-6">
          ...
          <div
            className={`p-4 rounded-lg ${
              shop.is_eligible ? 'bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800'
            }`}
          >
            <div className="flex items-center gap-2">
              {shop.is_eligible ? (
                <>
                  ...
                  <span className="font-medium text-green-800 dark:text-green-300">Eligible for Service</span>
                </>
              ) : (
                <>
                  <X className="w-5 h-5 text-red-600" aria-hidden="true" />
                  <span className="font-medium text-red-800 dark:text-red-300">Not Eligible</span>
                </>
              )}
            </div>
          ...
          <div className="bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-lg p-4">
            <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-4">Capacity Metrics</h3>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="text-center p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                <div className="text-2xl font-bold text-gray-900 dark:text-gray-100">{shop.backlog.cars_backlog}</div>
              ...
              <div className="text-center p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400">Weekly Inbound</div>
              </div>
              <div className="text-center p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                <div className="text-2xl font-bold text-green-600">
                  {shop.backlog.weekly_outbound}
                </div>
                <div className="text-xs text-gray-500 dark:text-gray-400">Weekly Outbound</div>
              </div>
            </div>
          </div>
          ...
          {hoursData.length > 0 && (
            <div className="bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-lg p-4">
              <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-4">Estimated Hours by Work Type</h3>
              <div className="space-y-3">
                {hoursData.map((item) => (
                  <div key={item.label}>
                    <div className="flex justify-between text-sm mb-1">
          ...
          {/* Rules Evaluation */}
          <div className="bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-lg p-4">
            <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-4">Rules Evaluation</h3>
            {Object.keys(groupedRules).length > 0 ? (
                      ...
                      {rules.map((rule, idx) => (
                        <div
                          ...
                          className={`flex items-center justify-between text-sm py-2 px-3 rounded ${
                            rule.result === 0 ? 'bg-red-50 dark:bg-red-900/20' : rule.result === 1 ? 'bg-green-50 dark:bg-green-900/20' : 'bg-gray-50 dark:bg-gray-800/50'
                          }`}
                        >
              ...
              <div className="space-y-2">
                {shop.failed_rules.map((rule) => (
                  <div key={rule.rule_id} className="bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 px-3 py-2 rounded">
              ...
              <p className="text-sm text-green-700 dark:text-green-400">All eligibility criteria met</p>
            )}
          </div>
        </div>

        {/* Footer Actions */}
        <div className="sticky bottom-0 bg-white dark:bg-gray-900 border-t dark:border-gray-700 px-6 py-4 flex items-center justify-between">
